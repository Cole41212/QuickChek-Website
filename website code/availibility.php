<?php
// We need - use sessions, so you should always start sessions using the below code.
session_start();
// If the user is not logged in redirect - the login page...
...
if (mysqli_connect_errno()) {
	exit('Failed - connect - MySQL: ' . mysqli_connect_error());
}
// We don't have the password or email info s-red in sessions, so instead, we can get the results from the database.
$stmt = $conn->prepare('SELECT password, fName, lName FROM accounts WHERE id = ?');
// In this case we can use the account ID - get the account info.
...
<body class="loggedin">
    ...
    <aside>
		...
		$sql = "SELECT * FROM availability WHERE username = '$username'";
		...
		$sql = "SELECT * FROM blackouts WHERE username = '$username'";
		$result = mysqli_query($conn, $sql);
		// Check if there are any rows returned
		if (mysqli_num_rows($result) > 0) {
			// Display the table header
